...
public class MainStage extends Controller {
	...
	public MainStage(List<Module> modules) throws JAXBException, IOException {
...
//		root = loader.load();
		...
		for (Module module : modules) {
			List<? extends JAXBEntity> entities = entityLoader.loadAllEntities(module.getEntityClass());

			if (entities.size() == 0)
				continue;
			if (module instanceof WorkspaceAPI) {
				WorkspaceAPI workspaceAPI = (WorkspaceAPI) module;
